...
function NewInvoiceComposer() {
  const searchParams = useSearchParams();
  const clientId = searchParams.get('clientId');

  return (
    <RedirectIfReadOnly href={routes.app.invoices}>
      <AppShell title="New invoice" fullWidth hideHeader>
        <div className="w-full">
          <InvoiceComposerModal open={true} onOpenChange={() => {}} mode="page" initialClientId={clientId} />
        </div>
      </AppShell>
    </RedirectIfReadOnly>
  );
