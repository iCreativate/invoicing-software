...
function ResetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(data.error?.message || 'Could not reset password. Request a new link.');
    } catch {
    ...
    <div className="min-h-screen w-full max-w-full bg-gradient-to-br from-blue-50 via-indigo-50 via-white to-purple-50 flex items-center justify-center px-3 sm:px-4 py-6 sm:py-12 box-border">
      <div className="w-full max-w-md bg-white rounded-2xl sm:rounded-3xl shadow-2xl p-6 sm:p-10 animate-fadeIn flex-shrink-0">
        ...
        <div className="text-center mb-8 sm:mb-10">
          ...
          <p className="text-sm sm:text-base text-gray-600">Choose a strong password for your account.</p>
        </div>

        {missingLink ? (
          <div className="rounded-xl border border-amber-200 bg-amber-50 text-amber-950 px-4 py-4 text-sm">
            <p className="mb-3">This page needs a valid reset link from your email.</p>
