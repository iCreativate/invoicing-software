...
export default function NewQuotePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.error?.message || 'Failed to create quote');
      }
    } catch (error) {
      ...
      const response = await fetch('http://localhost:5001/api/ai/generate-terms', {
        ...
        body: JSON.stringify({
          ...
          paymentTerms: formData.expiryDate && formData.issueDate 
            ? `${Math.ceil((new Date(formData.expiryDate).getTime() - new Date(formData.issueDate).getTime()) / (1000 * 60 * 60 * 24))} days`
            : undefined,
        }),
      });
  ...
  return (
    <DashboardLayout
      user={user}
      pageTitle="Create New Quote"
      pageDescription="Fill in the details to create a new quote"
      actionButton={
            <Link
        ...
        <div className="glass p-8 rounded-3xl shadow-xl border border-gray-200/50 animate-scaleIn">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">Terms & Conditions will be generated based on your company details and quote information</p>
                </div>
              </div>
            </div>
            ...
            <span className="absolute inset-0 bg-gradient-to-r from-blue-500 to-purple-500 opacity-0 group-hover:opacity-100 transition-opacity"></span>
                ...
                <span className="relative z-10">Create Quote</span>
                  </>
                )}
              </button>
              </div>
