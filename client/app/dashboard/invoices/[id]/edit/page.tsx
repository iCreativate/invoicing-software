...
export default function EditInvoicePage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.data.invoice) {
          ...
          setFormData({
            ...
            items: invoice.items && invoice.items.length > 0 
              ? invoice.items.map((item: any) => ({
                  ...
                  lineTotal: item.lineTotal || (item.quantity * item.unitPrice * (1 + (item.taxRate || 0) / 100)),
                }))
              : [{ description: '', quantity: 1, unitPrice: 0, taxRate: 0, lineTotal: 0 }],
            notes: invoice.notes || '',
            terms: invoice.terms || '',
          ...
          // Set template and colors if available
          if (invoice.template) {
            setSelectedTemplate(invoice.template);
          }
          if (invoice.colors) {
            setInvoiceColors(invoice.colors);
          }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.error?.message || 'Failed to update invoice');
        setLoading(false);
      }
    } catch (error: any) {
      console.error('Error updating invoice:', error);
      if (error.name === 'TypeError' && error.message.includes('fetch')) {
        alert(`Cannot connect to the server. Please ensure the backend is running at ${API_BASE} (e.g. "npm run dev" from project root).`);
      } else {
  ...
  const previewInvoiceNumber = formData.invoiceNumber || `INV-${new Date().getFullYear()}-00001`;
  ...
  return (
    <DashboardLayout
      ...
      pageDescription="Update invoice details"
      actionButton={
            <Link
    ...
    >
        ...
        <div className="glass p-8 rounded-3xl shadow-xl border border-gray-200/50 animate-scaleIn">
            ...
            <span className="absolute inset-0 bg-gradient-to-r from-blue-500 to-purple-500 opacity-0 group-hover:opacity-100 transition-opacity"></span>
                ...
                <span className="relative z-10">Update Invoice</span>
                  </>
                )}
              </button>
              </div>
